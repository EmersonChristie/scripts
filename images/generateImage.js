...
export const generateImage = async (artwork, options) => {
  ...
  let maxArtworkWidthPercent = 80; // Default max width percentage
...
//     longShadow: {
//       angle: 40,
//       length: 125 * intensityFactor, // length x width / 60
//       finalBlur: 65 * (2 - intensityFactor),
//       finalTransparency: 0.1 * intensityFactor,
//     },
//     shortShadow: {
//       angle: 35,
//       length: 90 * intensityFactor,
//       finalBlur: 40 * (2 - intensityFactor),
//       finalTransparency: 0.08 * intensityFactor,
//     },
//     upperShadow: {
//       angle: -62,
...
// const generateImage = async (artwork, options) => {
//   const { fileName, id, title, artworkHeight, artworkWidth } = artwork;
...
//   const imageSource = getDataURI(`${inputDir}${fileName}`);
//   const outputFileName = `${outputDir}${id}-${title}-${outputName}-${canvasWidth}x${canvasHeight}.${format}`;
...
//   let backgroundStyle;
//   const backgroundImagePath = `./static/background-images/${background}`;
...
//     const backgroundImageSource = getDataURI(backgroundImagePath);
//     backgroundStyle = `url(${backgroundImageSource})`;
//     // backgroundStyle = `url(${backgroundImagePath})`;
//   } else {
//     backgroundStyle = background; // Assume it's a gradient or color
//   }
...
//         }
//         #artwork {
//           max-width: {{maxArtworkWidth}};
//           max-height: {{maxArtworkHeight}};
//           box-shadow: ${generateArtShadows(7)};
...
//         .resize({ width: canvasWidth, height: canvasHeight, fit: "inside" })
//         .toFormat(getFormat(), { quality: 85 })
//         .toBuffer()
//         .then((data) => {
